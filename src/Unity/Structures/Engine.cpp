...
namespace Unity
{
	...
	void Vector3::ToVectors(Vector3* m_pForward, Vector3* m_pRight, Vector3* m_pUp)
	{
	...
	Quaternion Quaternion::Euler(float m_fX, float m_fY, float m_fZ)
	{
		float m_fDeg2Rad = static_cast<float>(M_PI) / 180.f;
	...
	Quaternion Quaternion::Euler(Vector3 m_vRot)
	{
